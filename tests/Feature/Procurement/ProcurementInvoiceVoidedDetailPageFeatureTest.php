...
final class ProcurementInvoiceVoidedDetailPageFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_admin_can_access_voided_procurement_invoice_detail_page_as_read_only(): void
    {
        ...
        $response->assertSee('Tidak ada aksi yang tersedia.');
        $response->assertSee('Nota sudah dibatalkan');
        $response->assertDontSee('Terima Barang');
        $response->assertDontSee('Status Penerimaan Barang');
    }
    ...
    private function user(string $role): User
    {
        $user = User::query()->create([
            'name' => 'Test',
            'email' => $role . '-voided-detail@example.test',
