...
final class PayrollTableDataValidationFeatureTest extends TestCase
{
    use RefreshDatabase;

    public function test_admin_gets_validation_error_when_sort_by_is_invalid(): void
    {
        $this->actingAs($this->admin())->getJson(route('admin.payrolls.table', ['sort_by' => 'invalid']))->assertStatus(422)->assertJsonValidationErrors(['sort_by']);
    }

    public function test_admin_gets_validation_error_when_per_page_is_invalid(): void
    {
        $this->actingAs($this->admin())->getJson(route('admin.payrolls.table', ['per_page' => 25]))->assertStatus(422)->assertJsonValidationErrors(['per_page']);
    }
