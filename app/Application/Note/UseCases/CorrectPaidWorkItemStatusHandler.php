...
use App\Application\Note\Services\{NoteCorrectionSnapshotBuilder, WorkItemStatusTransitionService};
...
final class CorrectPaidWorkItemStatusHandler
{
    ...
    public function handle(string $nId, int $line, string $status, string $reason, string $actorId): Result
    {
        $started = false;
        try {
            if ($line <= 0) throw new DomainException('Line number harus > 0.');
            $reason = trim($reason); $actorId = trim($actorId);
            if ($reason === '') return Result::failure('Alasan wajib diisi.', ['correction' => ['AUDIT_REASON_REQUIRED']]);
            if ($actorId === '') throw new DomainException('Actor wajib ada.');
            ...
            $this->audit->record('paid_work_item_status_corrected', [
                'performed_by_actor_id' => $actorId, 'note_id' => $note->id(), 'line_no' => $line,
                'target_status' => $item->status(), 'reason' => $reason, 'before' => $before, 'after' => $after
            ]);
