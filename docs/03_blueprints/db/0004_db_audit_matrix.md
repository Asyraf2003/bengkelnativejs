...
| `customer_payments` | `database/migrations/2026_03_14_000600_create_customer_payments_table.php`; `database/migrations/2026_04_27_000700_add_payment_method_and_cash_details_to_customer_payments.php` | Payment/source financial table | P0 payment source | FACT: `paid_at` is the payment/report date; must not be replaced by system timestamps | GAP: no separate system action timestamp proven beyond `paid_at` | GAP: no proven `created_at` / `updated_at` on `customer_payments`; GAP: `customer_payment_cash_details` also has no proven system timestamps | FACT: `amount_rupiah`, `payment_method`; FACT: cash detail table stores `amount_paid_rupiah`, `amount_received_rupiah`, `change_rupiah` | GAP: no actor/reason/audit linkage proven in this row | FACT: payment/refund baseline focused tests passed 13 tests, 45 assertions; FACT: writer audit found payment and cash detail inserts without system timestamps; RISK: direct inserts/fixtures may break under naive non-null timestamp patch | Medium residual: portable timestamp patch likely feasible, but PostgreSQL runtime is not active/proven | Medium residual: `customer_payment_cash_details` currently cascades on parent payment delete; local delete audit found no proven `customer_payments` hard-delete path except the FK cascade definition, so this remains a residual risk rather than a confirmed bug | Create narrow payment/refund timestamp patch blueprint next; preserve `paid_at` as payment/report date; do not add timestamp index without read-path proof; do not change cascade semantics without hard-delete proof and owner decision | No | Patch blueprint must define nullable/backfill policy, writer timestamp behavior, cash detail timestamp handling, direct fixture compatibility, allocation/reporting non-regression tests, and cascade-delete decision scope | Patch Blueprinted |
| `customer_refunds` | `database/migrations/2026_03_15_000100_create_customer_refunds_table.php` | Refund/source financial table | P0 refund source | FACT: `refunded_at` is the refund/report date; must not be replaced by system timestamps | GAP: no separate system action timestamp proven beyond `refunded_at` | GAP: no proven `created_at` / `updated_at` on `customer_refunds` | FACT: `amount_rupiah`, `customer_payment_id`, `note_id`, `reason`; FACT: indexes exist for payment, note, refunded date, and payment-note pair | FACT: `reason` exists; GAP: actor/audit linkage not proven in this row | FACT: payment/refund baseline focused tests passed 13 tests, 45 assertions; FACT: writer audit found refund insert without system timestamps; RISK: direct inserts/fixtures may break under naive non-null timestamp patch | Medium residual: portable timestamp patch likely feasible, but PostgreSQL runtime is not active/proven | Medium residual: refund allocations/reporting paths still need non-regression proof before schema patch | Create narrow payment/refund timestamp patch blueprint next; preserve `refunded_at` as refund/report date; do not add timestamp index without read-path proof | No | Patch blueprint must define nullable/backfill policy, writer timestamp behavior, refund allocation/reporting non-regression tests, direct fixture compatibility, and no report-date semantic change | Patch Blueprinted |
